...
- **Thesis:** Infer **one** clear through-line from the name + description (not every parallel subtopic). Broad policy-era topics read better with a single interpretive spine (e.g. how rules shifted from A to B over the period) unless the user asked for exhaustive coverage. **Biographical** rows should imply **one protagonist arc** (phase changes), not a scatter of milestones—avoid skeleton rows that only restate status (wealth figure, “still in role”) unless pivotal.
- **Imageable milestones:** Prefer **dated occurrences** (votes, hearings, filings, rallies, results, rulings) that could be illustrated as a scene—not rows of pure analysis (polling “signals,” donor-composition commentary, abstract “national impact”) unless tied to a **specific dated moment**.
- **Beat linkage & pacing:** Each milestone should be **one distinct shift** (one headline-worthy development per row). Order milestones **chronologically** so the list reads with **forward pull** where sources allow (context → escalation → turning point(s) → outcome)—not a flat, interchangeable list. Avoid **stacking** several milestones in the same short window that are the same wave split artificially—merge when they would not each change the reader’s understanding.
