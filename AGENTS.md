...
- **Matrix #:** The project has a **number** field **Matrix #** (1–44) aligned with [`docs/product/FEATURE_PRIORITIZATION_MATRIX.md`](docs/product/FEATURE_PRIORITIZATION_MATRIX.md) (that doc uses **numeric scores** for Information / Virality / Complexity—not L/M/H). **Table** view: sort or filter by **Matrix #** to match roadmap rows. After adding a new card, run `./scripts/gh-board-set-matrix.sh ISSUE MATRIX_ROW` or `./scripts/gh-board-set-matrix.sh --sync-all` (see [`docs/product/BOARD_PRIORITY.md`](docs/product/BOARD_PRIORITY.md)).
