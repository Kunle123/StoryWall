...
- **Biography vs issue story:** **Single-subject life/career** timelines should read like **one protagonist’s arc**—phase changes (founding → scale → pivot → public role) with beats as **turning points**, not a reference list; skip or merge weak “status confirmation” beats (e.g. net-worth-only rows, “continues as host”) unless essential. **Issue / policy explainers** keep StoryWall’s **context wedge** (“help me understand this issue”) with **one thesis**, but use the **same discipline**: each beat advances understanding, not parallel encyclopedia tracks.
...
- If the timeline description mentions a specific time period or date range (e.g., "2020 to 2025", "from 2015-2020", "between 2020 and 2025", "since 2020", "during 2020-2025"), you MUST distribute events across ALL years in that timespan
...
- CRITICAL: For media controversies, TV show incidents, or public figure controversies, you MUST use web search to find ALL specific incidents, quotes, dates, and documented controversies. Each specific incident should be a separate event
...
CRITICAL: Analyze the timeline description to identify the SPECIFIC THEME, TOPIC, or FOCUS. Then:
- Generate search queries that directly relate to that specific theme/topic (NOT generic queries)
- Focus events ONLY on incidents that directly relate to the identified theme/topic
- If the description mentions a specific type of criticism, controversy, or incident, search for and include ONLY events that match that specific type
- DO NOT include general complaints, statistics, or incidents unrelated to the specific theme - ONLY include events that directly relate to the timeline's focus
- Example: If timeline is "criticizing the working classes", search for "[show] working class criticism", "[show] dismissive working class", etc. and include ONLY incidents about working-class criticism
- Example: If timeline is "climate change denial", search for "[show] climate change denial", "[show] climate skeptic", etc. and include ONLY incidents about climate denial
- Example: If timeline is "sexist comments", search for "[show] sexist", "[show] misogynistic", etc. and include ONLY incidents about sexism
