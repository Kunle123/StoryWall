...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error('[Twitter OAuth1 Callback] Failed to get base URL:', error);
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        // Only clear tokens if we get a specific permissions error (403), not code 215
        // Code 215 in verification might just be a signature encoding issue, not a permissions problem
        // Since tokens were successfully obtained, we should keep them and let the actual upload test permissions
        if (!verification.hasWritePermissions) {
          const isCode215 = verification.error?.includes('code: 215') || verification.error?.includes('Bad Authentication data');
          
          if (isCode215) {
            // Code 215 in verification might be a signature encoding issue, not permissions
            // Keep the tokens - they were successfully obtained, so they should work for uploads
            console.warn('[Twitter OAuth1 Callback] ⚠️  Verification failed with code 215 (signature issue), but tokens were successfully obtained');
            console.warn('[Twitter OAuth1 Callback] ⚠️  Keeping tokens - actual upload will test permissions');
    ...
    try {
      ...
      console.warn('[Twitter OAuth1 Callback] Invalid baseUrl, using production default:', finalBaseUrl);
    }
