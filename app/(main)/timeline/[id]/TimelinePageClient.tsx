...
export type TimelinePageClientProps = {
  initialTimeline: any;
  initialEvents: TimelineEvent[];
};

const TimelinePageClient = ({
  initialTimeline,
  initialEvents,
  ...
  // Keep local state in sync when navigating between timelines (client-side)
  useEffect(() => {
    ...
    async function hydrateFromServerOrFetch() {
      ...
      if (initialTimeline) {
        ...
        if (isEditMode && isSignedIn && user) {
          const timelineCreatorId =
            initialTimeline.creator?.id || initialTimeline.creator_id;
          ...
          try {
            ...
            if (userResponse.ok) {
              ...
              const isAdmin = userEmail === "kunle2000@gmail.com";
      ...
      // Fallback: client-only fetch (e.g. edge cases without server data)
      try {
        setLoading(true);
        setError(null);
        ...
        if (timelineResult.data) {
          ...
          if (isEditMode && isSignedIn && user) {
            const timelineCreatorId =
              timelineResult.data.creator?.id || timelineResult.data.creator_id;
            const userEmail = user.primaryEmailAddress?.emailAddress;
            try {
              const userResponse = await fetch("/api/user/profile");
              if (userResponse.ok) {
          ...
          if (eventsResult.data && eventsResult.data.length > 0) {
            const transformedEvents = eventsResult.data.map(
            ...
            {initialTimeline ? (
              <>
                <h1 className="text-2xl font-bold font-display text-foreground">
                  {initialTimeline.title}
                </h1>
                {initialTimeline.description && (
                  <p className="text-sm text-muted-foreground mt-2 line-clamp-4">
                    {initialTimeline.description}
                  </p>
                )}
                <p className="text-xs text-muted-foreground mt-6">
                  {!isLoaded || !browseGateOk
                    ? "Preparing timeline…"
                    : "Loading…"}
                </p>
              </>
